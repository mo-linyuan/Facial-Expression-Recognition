import os
import argparse
os.environ["TF_CPP_MIN_LOG_LEVEL"] = "2"#屏蔽tensorflow的log信息
import cv2
import numpy as np
...
parser.add_argument("--video_path", type=str, default='')#./assets/zhou.mp4
    ...
    while True:
        ...
        if faces is not None and len(faces) > 0:
            for (x, y, w, h) in faces:
                ...
                frame = cv2_img_add_text(frame, emotion, x+30, y+30, font_color, 25)
                # puttext中文显示问题
                # cv2.putText(frame, emotion, (x + 30, y + 30), cv2.FONT_HERSHEY_SIMPLEX, 1, font_color, 4)
        cv2.imshow("Face Emotion Recognition", cv2.cvtColor(frame, cv2.COLOR_RGB2BGR))  # 利用人眼假象
